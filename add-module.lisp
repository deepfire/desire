...
(defun add-module (url &optional module-name &key systemlessp (system-type *default-system-type*))
  ...
  (multiple-value-bind (remote-type distributor-name port raw-path) (parse-remote-namestring url)
    (let* ((module-name (or module-name (make-keyword (string-upcase (lastcar raw-path)))))
      ...
      (multiple-value-bind (distributor created-distributor-p) (or (prog1 (find-distributor-fuzzy distributor-name)
                                                                     (format t ";; Found ~A, looking for a corresponding remote..~%" (name distributor)))
  ...
  (let ((module (if remote-nickname
    ...
    (when auto-lust
      (handler-case (lust (name module))
        (error (c) "~@<There was an error stealing ~S~:[~; ~:*~A~]:~_~A~_Correspondinly, created resources were released.~:@>" url-or-name remote-nickname c)))))