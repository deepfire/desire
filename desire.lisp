...
(defun desire (desires &key skip-present (seal-p t))
  ...
  (let* ((interpreted-desires (mapcar (curry #'xform-if-not #'consp (lambda (m) (list (name (module-best-distributor m)) m))) desires)))
    ...
    (let* ((*desires* (substitute-desires *desires* (remove-if-not #'consp desires)))
      ...
      (multiple-value-bind (success system-vocabulary) (desire-do-one-step (mapcar #'fetch-anyway desired-list) skip-present nil)
        (declare (ignore success))
        (let ((missing-system-names (mapcar #'car (remove t system-vocabulary :key #'cdr))))
          (when-let ((missing (set-difference missing-system-names *implementation-provided-systems* :test #'string=)))
