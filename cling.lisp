...
;; (defmethod fetch ((locality-to git-locality) (locality-from git-locality) module &aux (name (down-case-name module)))
;;   (let ((from-repo-dir (subdirectory from-locality name))
;;         (to-repo-dir (subdirectory to-locality name)))
;;     (unless (directory-exists-p repo-dir)
;;       (within-directory (dir repo-dir :if-exists :error :if-does-not-exist :create)
;;         (git "init-db")
;;         (git "remote" "add" (down-case-name locality-from) (url remote name))))
;;     (within-directory (dir (subdirectory locality name))
...
(defmethod fetch ((locality git-locality) (remote git-remote) module &aux (name (down-case-name module)))
  (let ((repo-dir (subdirectory locality name)))
    (unless (directory-exists-p repo-dir)
      (within-directory (dir repo-dir :if-exists :error :if-does-not-exist :create)
        (git "init-db")
...
(defmethod fetch ((locality git-locality) (remote hg-http-remote) module &aux (name (down-case-name module)))
  (let ((hg-repo-dir (subdirectory (master-mirror 'hg) name))
        (git-repo-dir (subdirectory locality name)))
    ))

(defmethod fetch ((locality git-locality) (remote darcs-http-remote) module &aux (name (down-case-name module)))
  (let ((darcs-repo-dir (subdirectory (master-mirror 'darcs) name))
        (git-repo-dir (subdirectory locality name))
...
(defmethod fetch ((locality git-locality) (remote cvs-rsync-remote) module &aux (name (down-case-name module)))
  (let* ((cvs-locality (master-mirror 'cvs))
         (cvs-repo-dir (subdirectory cvs-locality name))
         (git-repo-dir (subdirectory locality name))
         (url (url remote module)))
...
(defmethod fetch ((locality git-locality) (remote svn-rsync-remote) module &aux (name (down-case-name module)))
  (let ((svn-repo-dir (subdirectory (master-mirror 'svn) name))
        (git-repo-dir (subdirectory locality name))
        (url (url remote module)))
