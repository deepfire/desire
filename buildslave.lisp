...
(defgeneric run-module-test (type module)
  (:method :around (type module)
    (with-recorded-status (:record-backtrace t)
      (unwind-protect (call-next-method)
        (finish-output *standard-output*)
        (finish-output *error-output*))))
  (:method ((o (eql :master-reachability-phase)) m)
    (touch-module m))
  (:method ((o (eql :master-update-phase)) m)
    (let ((*fetch-errors-serious* t))
      (update m)
      t))
  (:method ((o (eql :slave-fetch-phase)) m)
    (let ((*fetch-errors-serious* t)
  ...
  (:method ((o (eql :slave-load-phase)) m)
    ...
    t)
  (:method ((o (eql :slave-test-phase)) m)
  ...
  (let ((modules (mapcar #'module module-names))
    ...
    (iter (for p in phases)
          (iter (for m in modules)
                ...
                (destructuring-bind (&key return-value condition backtrace) (run-module-test p m)
